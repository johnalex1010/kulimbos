	...
	const Kulimbos = {
		...
		mobileMenu: {
			...
			handleToggle() {
				...
				this.toggle.setAttribute(
					"aria-label",
					this.isOpen ? "Cerrar menu" : "Abrir menu",
				);
			},
		...
		heroSlider: {
			...
			goTo(index) {
				...
				this.slides[this.current].classList.remove(
					"home-hero__slide--active",
				);
				if (this.dots[this.current]) {
					this.dots[this.current].classList.remove(
				...
				this.current = index;
				this.slides[this.current].classList.add(
					"home-hero__slide--active",
				);
				if (this.dots[this.current]) {
					this.dots[this.current].classList.add(
						"home-hero__dot--active",
					);
					this.dots[this.current].setAttribute(
						"aria-selected",
		...
		productGallery: {
			...
			setupGallery(gallery) {
				...
				const thumbs = Array.from(
					gallery.querySelectorAll("[data-product-gallery-thumb]"),
				);
				...
				let current = thumbs.findIndex((thumb) =>
					thumb.classList.contains("is-active"),
				);
				current = current >= 0 ? current : 0;
				let visibleStart =
					Math.floor(current / visibleThumbs) * visibleThumbs;
				...
				const updateVisibleThumbs = () => {
					...
					visibleStart = Math.max(
						0,
						Math.min(visibleStart, thumbs.length - visibleThumbs),
					);
					...
					thumbs.forEach((thumb, thumbIndex) => {
						...
						if (thumbItem) {
							thumbItem.hidden =
								thumbIndex < visibleStart ||
								thumbIndex >= visibleStart + visibleThumbs;
						}
					});
				};
		...
		productQuantity: {
			...
			setupControl(control) {
				...
				const whatsappLink = actions
					? actions.querySelector("[data-product-whatsapp]")
					: null;

				if (!input || !decreaseBtn || !increaseBtn) return;

				const minAttribute = Number.parseInt(
					input.getAttribute("min"),
					10,
				);
				const min = Number.isNaN(minAttribute) ? 1 : minAttribute;
				const maxAttribute = Number.parseInt(
					input.getAttribute("max"),
					10,
				);
				const max = Number.isNaN(maxAttribute)
					? Number.POSITIVE_INFINITY
					: Math.max(min, maxAttribute);
				...
				const updateWhatsappLink = () => {
					if (!whatsappLink || !whatsappLink.dataset.whatsappMessage)
						return;

					const quantity = Number.parseInt(input.value, 10) || min;
					const message =
						whatsappLink.dataset.whatsappMessage.replace(
							/Cantidad:\s*\d+/u,
							`Cantidad: ${quantity}`,
						);
					whatsappLink.href = `https://api.whatsapp.com/send?phone=573144681188&text=${encodeURIComponent(message)}`;
				};

				const normalize = () => {
					const value = Number.parseInt(input.value, 10);
					input.value = String(
						Number.isNaN(value) ? min : clampQuantity(value),
					);
					updateWhatsappLink();
					updateButtonState();
				};
				...
				decreaseBtn.addEventListener("click", () => {
					normalize();
					input.value = String(
						clampQuantity(Number.parseInt(input.value, 10) - 1),
					);
					updateWhatsappLink();
					updateButtonState();
		...
		cart: {
			...
			init() {
				const cartConfig =
					window.kulimbosData && window.kulimbosData.cart
						? window.kulimbosData.cart
						: {};
				this.storageKey = cartConfig.storageKey || this.storageKey;
				this.shippingFee =
					Number.parseInt(cartConfig.shippingFee, 10) ||
					this.shippingFee;
			...
			getQuantityFromButton(button) {
				...
				const input = document.querySelector(
					button.dataset.cartQuantityInput,
				);
				if (!input) return 1;
			...
			getProductFromButton(button) {
				...
				const stock = Number.parseInt(
					button.dataset.cartProductStock,
					10,
				);
					...
					.forEach((button) => {
						button.addEventListener("click", () => {
							...
							const added = this.addItem(
								this.getProductFromButton(button),
							);
							if (!added) return;
			...
			bindCartPage() {
				...
				const clearButton =
					this.cartPage.querySelector("[data-cart-clear]");
				if (clearButton) {
			...
			createItemElement(item) {
				...
				price.textContent = this.formatCurrency(
			...
			renderCartPage() {
				...
				const clearButton =
					this.cartPage.querySelector("[data-cart-clear]");
				const checkoutButton = this.cartPage.querySelector(
					"[data-cart-checkout]",
				);
				const subtotal = this.getSubtotal();
				...
				if (checkoutButton) {
					...
					checkoutButton.href = hasItems
						? `https://api.whatsapp.com/send?phone=573144681188&text=${encodeURIComponent(this.getCheckoutMessage())}`
						: "#";
				}

				this.cartPage
					.querySelectorAll("[data-cart-summary-count]")
					.forEach((node) => {
						node.textContent = `(${quantity})`;
					});
				this.cartPage
					.querySelectorAll("[data-cart-summary-subtotal]")
					.forEach((node) => {
						node.textContent = this.formatCurrency(subtotal);
					});
				this.cartPage
					.querySelectorAll("[data-cart-summary-shipping]")
					.forEach((node) => {
						node.textContent = this.formatCurrency(shipping);
					});
				this.cartPage
					.querySelectorAll("[data-cart-summary-total]")
					.forEach((node) => {
						node.textContent = this.formatCurrency(
							subtotal + shipping,
						);
					});
			},
		...
		favorites: {
			...
			init() {
				const favoriteConfig =
					window.kulimbosData && window.kulimbosData.favorites
						? window.kulimbosData.favorites
						: {};
			...
			updateHeaderCount() {
				document
					.querySelectorAll(
						"#favorites-count, [data-favorites-count]",
					)
					.forEach((badge) => {
			...
			bindFavoritesPage() {
				this.favoritesPage = document.querySelector(
					"[data-favorites-page]",
				);
				if (!this.favoritesPage) return;

				const search = this.favoritesPage.querySelector(
					"[data-favorites-search]",
				);
				const sort = this.favoritesPage.querySelector(
					"[data-favorites-sort]",
				);
				const clear = this.favoritesPage.querySelector(
			...
			renderFavoritesPage() {
				...
				const list = this.favoritesPage.querySelector(
					"[data-favorites-items]",
				);
				const empty = this.favoritesPage.querySelector(
					"[data-favorites-empty]",
				);
				const clear = this.favoritesPage.querySelector(
					"[data-favorites-clear]",
				);
				const count = this.favoritesPage.querySelector(
					"[data-favorites-summary-count]",
				);
				...
				if (list) {
					...
					visibleItems.forEach((item) =>
						list.appendChild(this.createFavoriteElement(item)),
					);
		...
		plushiesFilters: {
			...
			init() {
				this.roots = Array.from(
					document.querySelectorAll("[data-plushies-listing]"),
				);
			...
			setupFilters(root) {
				...
				const cards = Array.from(
					root.querySelectorAll("[data-filter-card]"),
				);
				...
				const priceLabel = root.querySelector(
					"[data-filter-price-label]",
				);
				const pagination = root.querySelector(
					"[data-filter-pagination]",
				);
				const categoryButtons = Array.from(
					root.querySelectorAll("[data-filter-category]"),
				);
				const colorButtons = Array.from(
					root.querySelectorAll("[data-filter-color]"),
				);
				const checkboxes = Array.from(
					root.querySelectorAll("[data-filter-checkbox]"),
				);
				const viewButtons = Array.from(
					root.querySelectorAll("[data-view-mode]"),
				);
				...
				const getCheckedValues = (type) =>
					checkboxes
						.filter(
							(checkbox) =>
								checkbox.dataset.filterCheckbox === type &&
								checkbox.checked,
						)
						.map((checkbox) => checkbox.value);

				const getActiveCategory = () => {
					const active = categoryButtons.find((button) =>
				...
				const sortCards = (visibleCards) => {
					...
					sorted.sort((cardA, cardB) => {
						...
						if ("name-asc" === mode)
							return cardA.dataset.name.localeCompare(
								cardB.dataset.name,
								"es",
							);
						return (
							Number(cardB.dataset.popularity) -
				...
				const applyFilters = (options = {}) => {
					...
					const isPriceFilterActive =
						priceMax && priceMax.value !== priceMax.max;
					...
					const visibleCards = cards.filter((card) => {
						...
						const priceMatches =
							!isPriceFilterActive ||
							(card.dataset.hasPrice === "true" &&
								Number(card.dataset.price) <= maxPrice);
						const ageMatches = matchesAny(
							card.dataset.age,
							selectedAges,
						);
						const sizeMatches = matchesAny(
							card.dataset.size,
							selectedSizes,
						);
						const colorMatches = matchesAny(
							card.dataset.color,
							activeColors,
						);
						const materialMatches = matchesAny(
					...
					const totalPages = Math.max(
						1,
						Math.ceil(sortedCards.length / pageSize),
					);
					...
					clear.addEventListener("click", () => {
						categoryButtons.forEach((button) =>
							button.classList.toggle(
								"is-active",
								"all" === button.dataset.filterCategory,
							),
						);
						colorButtons.forEach((button) =>
				...
				if (toggle && layout) {
					toggle.addEventListener("click", () => {
						const isHidden =
							layout.classList.toggle("is-filter-hidden");
		...
		testimonialsSlider: {
			...
			init() {
				...
				this.cards = Array.from(
					this.root.querySelectorAll(".testimonial-card"),
				);
				this.prevBtn = this.root.querySelector(
					"[data-testimonials-prev]",
				);
				this.nextBtn = this.root.querySelector(
			...
			handleResize() {
				...
				this.visible = Math.max(
					1,
					parseInt(
						styles.getPropertyValue("--testimonial-visible"),
						10,
					) || 1,
				);
